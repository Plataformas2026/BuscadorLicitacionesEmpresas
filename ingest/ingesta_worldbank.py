...
# fila de cabecera comprobando que la 1a celda no diga "Solicitation
# Title".
...
def construir_registro(item: dict) -> dict:
    ...
    pais = _detectar_pais(item.get("titulo"))

    partes_descripcion = []
    if item.get("number"):
        partes_descripcion.append(f"Nº de solicitación: {item['number']}.")
    descripcion = " ".join(partes_descripcion) or None

    number = (item.get("number") or "").strip()
    slug_base = number or _generar_slug(item.get("titulo") or item["url_oficial"])
...
def ejecutar_sincronizacion():
    ...
    en_ventana = [
        ...
        if n.get("fecha_publicacion") and desde <= date.fromisoformat(n["fecha_publicacion"]) <= hoy
    ]
